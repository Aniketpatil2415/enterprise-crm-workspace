...
export default function Dashboard() {
  ...
  // Securely fetch leads from MySQL database via our Node.js API
  const fetchLeads = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/leads', {
        headers: {
          'x-user-id': currentUser.uid // Security token passed to the backend bouncer
        }
      });
  ...
  // Listen for Firebase Authentication state before firing API calls
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        setLeads([]);
        setLoadingLeads(false);
        navigate('/auth'); // Redirect if session expires
      }
    });
  ...
  // Dynamic calculations from database text records
  ...
  return (
    <div className="min-h-screen flex bg-brand-900 text-white overflow-hidden relative">
      {/* Premium Ambient Background Blur Orbs */}
      ...
      {/* Sidebar - Solid Enterprise Glass Structure */}
      <aside className="w-64 glass-panel border-l-0 border-t-0 border-b-0 rounded-none relative z-10 flex flex-col">
        ...
        <div className="p-4 border-t border-glass-border">
          <button 
            ...
            className="w-full flex items-center px-4 py-3 text-red-400 hover:bg-red-400/10 rounded-lg transition-all"
          >
      ...
      <main className="flex-1 flex flex-col relative z-10 h-screen overflow-y-auto">
        ...
        <div className="p-8">
          <div className="flex justify-between items-center mb-8">
            ...
            {/* The Trigger Button for our new Lead Engine */}
            <button 
          ...
          {/* Live Updating Analytical Stats Grid */}
          ...
          {/* Conditional Rendering Layer based on Data Presence */}
          {loadingLeads && !userInit ? (
            <div className="flex justify-center items-center h-64">
              <Loader2 className="w-8 h-8 text-brand-400 animate-spin" />
            </div>
          ) : leads.length > 0 ? (
            /* Premium Frosted Glassmorphic Data Table */
            <div className="glass-panel overflow-hidden border border-glass-border shadow-xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-glass-border bg-brand-800/40 text-xs font-semibold uppercase tracking-wider text-gray-400">
            ...
            /* Empty Placeholder Board Overlay */
      ...
      {/* Embedded Modal Component Layer */}
      <AddLeadModal 
        isOpen={isModalOpen} 
        onClose={() => setIsModalOpen(false)} 
        onSuccess={fetchLeads} // Automatically auto-refreshes the table without reloading the app
      />
    </div>
  );
}